...
namespace TextAdventure.IntegrationTests.FontGenerator
{
	...
	public class FontGenerator
	{
		...
		public void GenerateFont()
		{
			...
			Image fontImage = new Bitmap(characterSize.Width * 16, characterSize.Height * 16, PixelFormat.Format32bppArgb);

			using (Graphics graphics = Graphics.FromImage(fontImage))
			{
				graphics.FillRectangle(Brushes.Black, new Rectangle(new Point(0, 0), fontImage.Size));
				...
				for (int y = 0; y < 16; y++)
				{
					for (int x = 0; x < 16; x++, index++)
					{
						...
						TextRenderer.DrawText(
							...
							character,
							font,
							new Rectangle(x * characterSize.Width, y * characterSize.Height, characterSize.Width, characterSize.Height),
							Color.White,
							Color.Black,
