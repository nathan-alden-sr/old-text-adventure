...
namespace TextAdventure.WindowsGame.Components
{
	...
	{
		public const string SelectedAnswerTextFormat = "  {0}  ";
		public const string UnselectedAnswerTextFormat = "  {0}  ";
		...
		public MessageTextFormatter(IMessage message, SpriteFont font, WindowTexture selectedAnswerWindowTexture, float maximumLineWidth)
		{
		...
		private MessageTextAnswer GetMessageTextAnswer(IMessageQuestion question, IMessageAnswer answer)
		{
			return new MessageTextAnswer(
				answer,
				answer.Text,
				_font.MeasureString(answer.Text),
